...
class QTTTestRuleRule : public AbstractASTVisitorRule<QTTTestRuleRule>
{
public:
...
.. code-block:: cpp
    ...
    */
    ...
    bool VisitObjCContainerDecl(ObjCAbstractSourceCodeReaderRuleContainerDecl *node)
    {
        return true;
    }
     */
    ...
    {
        cout<<"ObjCPropertyDecl node-->"<<node->getNameAsString()<<endl;

        cout<<"isBlockPointerType node-->"<<node->getType()->isBlockPointerType()<<endl;
        
        ObjCPropertyDecl::PropertyAttributeKind attributeKind = node->getPropertyAttributes();
        cout<<"attributeKind getName-->"<<node->getType().getAsString() <<endl;
        cout<<"attributeKind isAtomic-->"<<node->isAtomic() <<endl;
        ...
        if(node->getTypeSourceInfo()){
            ...
            if(typeStr.find("NSString*")!=string::npos || typeStr.find("NSArray*")!=string::npos){
                if(!(attributeKind & ObjCPropertyDecl::OBJC_PR_copy)){
                    if(typeStr.find("NSString*")!=string::npos)
                    {
                        ...
                        addViolation(node, this, "NSString应尽量用copy修饰，避免用strong修饰");
                    }
                }
            }
            ...
            //Delegate
                ...
                string typeSrcCode;
                typeSrcCode.assign(context->getSourceManager().getCharacterData(node->getSourceRange().getBegin()),node->getSourceRange().getEnd().getRawEncoding()-node->getSourceRange().getBegin().getRawEncoding());
                ...
                if(replaceName.compare(typeSrcCode)){
                    ...
                    SourceLocation nameStart = node->getSourceRange().getBegin();
...
static RuleSet rules(new QTTTestRuleRule());
